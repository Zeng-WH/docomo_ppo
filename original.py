...
def main():
    ...
    gpt2_model = BARTHeadWithValueModel.from_pretrained('facebook/bart-large-cnn').cuda()
    #gpt2_model_ref = BARTHeadWithValueModel.from_pretrained('facebook/bart-large-cnn').cuda()
    gpt2_tokenizer = BartTokenizer.from_pretrained('facebook/bart-large-cnn')



    # initialize trainer
    #ppo_config = {'batch_size': 1, 'forward_batch_size': 1, 'lr': 5e-6}
